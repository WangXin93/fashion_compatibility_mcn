...
class CompatModel(nn.Module):
    def __init__(self, embed_size=1000, need_rep=False, vocabulary=None):
        """Self attention model for outfit compatibility prediction.
        ...
        nn.init.xavier_uniform_(self.qlinear.weight)
        nn.init.constant_(self.qlinear.bias, 0)
        nn.init.xavier_uniform_(self.klinear.weight)
        nn.init.constant_(self.klinear.bias, 0)
    ...
    def _compute_score(self, images, activate=True):
        ...
        query = self.qlinear(features)
        key = self.klinear(features)
        d_k = query.size(-1)
        scores = torch.matmul(query, key.transpose(-2, -1)) / np.sqrt(d_k)
        p_attn = torch.softmax(scores, dim=-1)
        features = torch.matmul(p_attn, features)
